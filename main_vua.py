...
def main():
    ...
    parser.add_argument("--processes_num", type=int, default=10,
    ...
    parser.add_argument("--concat_mode",
                        choices=["concat", "graph_attention"],
                        required=True,
    ...
    label_columns = read_cataloge(args.train_path)

    #set embedding
    embeddings = get_embedding_matrix(args, vocab, normalization=False)
    embeddings_entity = get_triples_embedding_matrix(args, os.path.join(args.triples_vocab_path, "entity_transE.txt"), vocab_entity, normalization=False)
    ...
    def evaluate(args, is_test):
        ...
        triples_ids = torch.LongTensor([example[1] for example in dataset]).cuda()
        id2_ids = torch.LongTensor([example[2] for example in dataset]).cuda()
        label_ids = torch.LongTensor([example[3] for example in  dataset]).cuda()
        length_ids = torch.LongTensor([example[4] for example in  dataset]).cuda()
        ...
        for i, (input_ids_batch, triples_ids_batch, id2_ids_batch, label_ids_batch, length_ids_batch) in enumerate(
                batch_loader(batch_size, input_ids, triples_ids, label_ids, length_ids, id2_ids)):
            model.zero_grad()
            ...
            if args.attention_layer == 'att':
                predicted, weight = model(input_ids_batch, triples_ids_batch, length_ids_batch, elmo_embedding,
                                               id2_ids_batch)
            else:
                predicted, weight, _ = model(input_ids_batch, triples_ids_batch, length_ids_batch, elmo_embedding,
                                               id2_ids_batch)
    ...
    def train():
        ...
        trainset = read_dataset(args, args.train_path, label_columns, vocab, vocab_entity, vocab_relation)
        random.shuffle(trainset)

        best_josn['train_num'] = len(trainset)

        input_ids = torch.LongTensor([example[0] for example in trainset]).cuda()
        triples_ids = torch.LongTensor([example[1] for example in trainset]).cuda()
        id2_ids = torch.LongTensor([example[2] for example in trainset]).cuda()
        label_ids = torch.LongTensor([example[3] for example in trainset]).cuda()
        length_ids = torch.LongTensor([example[4] for example in trainset]).cuda()
        ...
        for epoch in range(1, args.epochs_num + 1):
            model.train()
            for i, (input_ids_batch, triples_ids_batch, id2_ids_batch, label_ids_batch, length_ids_batch) in enumerate(
                    batch_loader(args.batch_size, input_ids, triples_ids, label_ids, length_ids, id2_ids)):
                model.zero_grad()
                input_ids_batch = input_ids_batch.cuda()
                label_ids_batch = label_ids_batch.cuda()
                triples_ids_batch = triples_ids_batch.cuda()
                ...
                if args.attention_layer == 'att':
                    predicted_ids_batch, _ = model(input_ids_batch, triples_ids_batch, length_ids_batch, elmo_embedding, id2_ids_batch)
                else:
                    predicted_ids_batch, _, orthogonal_loss = model(input_ids_batch, triples_ids_batch, length_ids_batch, elmo_embedding, id2_ids_batch)
